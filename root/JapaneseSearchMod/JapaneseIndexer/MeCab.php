...
/**
 * JapaneseIndexer_MeCab is an indexer class for Japanese, using MeCab as engine for wakachigaki. ( MeCab : Yet Another Part-of-Speech and Morphological Analyzer )
 *
...
class JapaneseIndexer_MeCab extends JapaneseIndexer_AL
{
	 ...
	 * @param string $mode 'Ext' or 'CLI' or null(autofind)
	 */
	...
	{
		$encodings = array('utf-8', 'euc-jp', 'shift-jis');
...
class JapaneseIndexer_MeCabExt extends JapaneseIndexer_MeCabBase
{
	...
	static function test_wakachigaki($encoding, $dicdir)
	{
		...
		if ($dicdir != '')
		{
			$engine = mecab_new(array('-d', $dicdir));
		}
		else
		{
			$engine = mecab_new();
	...
	private function set_engine()
	{
		...
		{
			$this->engine = mecab_new(array('-d', $this->config['dicdir']));
		}
		else
		{
			$this->engine = mecab_new();
		}
		register_shutdown_function(array($this, 'mecab_destroy'));
	}
...
class JapaneseIndexer_MeCabBase
{
	...
	protected function assemble_wakachigaki($text_pieces)
	{
		...
		foreach($text_pieces as $i => $piece)
		{
			...
			($hinshi == '記号' && $wakachigaki_level >= JapaneseIndexer_MeCabData::WAKACHIGAKI_LEVEL_KIGOU) ||
			($hinshi == '名詞' && $wakachigaki_level >= JapaneseIndexer_MeCabData::WAKACHIGAKI_LEVEL_MEISHI) ||
			($hinshi == '助詞' && $wakachigaki_level >= JapaneseIndexer_MeCabData::WAKACHIGAKI_LEVEL_JOSHI)
			)
			{
				$wakachigaki .= ' ' . $surface;
			}
			else if (
			($previous_hinshi == '記号' && $wakachigaki_level >= JapaneseIndexer_MeCabData::WAKACHIGAKI_LEVEL_KIGOU) ||
			($previous_hinshi == '名詞' && $wakachigaki_level >= JapaneseIndexer_MeCabData::WAKACHIGAKI_LEVEL_MEISHI) ||
			($previous_hinshi == '助詞' && $wakachigaki_level >= JapaneseIndexer_MeCabData::WAKACHIGAKI_LEVEL_JOSHI)
			)
			{
				$wakachigaki .=  ' ' . $surface;
			}
