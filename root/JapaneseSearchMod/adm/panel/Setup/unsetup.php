<?php
class phpBB3_JapaneseSearchModAdmPanel_Setup_unsetup extends phpBB3_JapaneseSearchModAdmPanel_BaseMenu {
	function main($current_page) {
		...
		if (! $this->setup_done) {
			$template->assign_vars ( array (
					'INFORMATION_MESSAGE' => sprintf ( sprintf ( $user->lang ['JSM_SETUPPANEL_SETUP_NOT_DONE'], phpBB3_JapaneseSearchModMain::VERSION ) ) 
			) );
		} else {
	...
	protected function create_linear_block($block, $block_type = '1') {
		parent::create_linear_block ( $block, 'SETUPPANEL_UNSETUP', $block_type );
	}
}
