...
class phpBB3_JapaneseSearchModHighlight extends phpBB3_JapaneseSearchModFulltextNativeJa {
	...
	function highlightFilter($hilit) {
		...
		if (! $this->is_FulltextNativeJa) {
			// ネイティブコード
			return $hilit = implode ( '|', explode ( ' ', preg_replace ( '#\s+#u', ' ', str_replace ( array (
					'+',
					'-',
					'|',
					'(',
	 ...
	 *
	 * @param string $hilit
	 *        	'KOKIA|ありがとう' 等。 パイプライン(|) でキーワードが区切られていることが前提。各キーワードは既に preg_quote() されている場合がある
	 * @param string $text        	
	 * @param string $css        	
	 * @return string
	 */
